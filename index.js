...
client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `EAI @${num.split('@')[0]}\ BEM VINDO/A AO GRUPO *${mdata.subject}* LÊ AS REGRAS PARA EVITAR BAN`
				...
				teks = `VLW MANO @${num.split('@')[0]} NEM FAZIA FALTA KKK`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	client.on('message-new', async (mek) => {
		try {
			...
			mess = {
				wait: '❬❗❭AGUEADER CASO DEMORA TENTE DE NOVO❬❗❭',
				success: '️❬ ✔ ❭ PRONTO',
				error: {
				...
				only: {
					group: '❬❗❭ COMANDO SÓ PODE SER FEITO EM GRUPOS❬❗❭ ',
					ownerG: '❬❗❭ TU E BURRO MANO NÃO VE QUE E SO PRO PROPRIETÁRIO DO BOT.❬❗❭ ',
					ownerB: '❬❗❭ TU E BURRO MANO NÃO VE QUE E SO PRO PROPRIETÁRIO DO BOT.❬❗❭  ',
					admin: ' ❬ ⚠️ ❭ COMANDO PERMITIDO SOMENTE PARA ADEMIR ❬ ⚠️ ❭ ',
			...
			switch(command) {
				...
				case 'info':
					...
					break
				case 'listabloq': 
					teks = 'DEI BLOCK MEMO/DEI BLOCK MEMO'
...
case 'lofi':
                ...
                case 'reportarbug':
                    ...
                    reply('PDP PARSA, BUG FOI REPORTADO, SE FOR MENTIRA SEU CHIP VAI VIRA FUMAÇA👺🤡')
                    break
					case 'marcar':
					members_id = []
